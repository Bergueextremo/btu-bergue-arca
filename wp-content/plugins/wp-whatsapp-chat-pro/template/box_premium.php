<div id="qlwapp" data-autoloadelay="<?php echo esc_attr( $box['auto_delay_open'] ); ?>" class="qlwapp-premium <?php echo esc_attr( $box['auto_open'] ) == 'yes' ? 'auto-load ' : '';
  printf( "qlwapp-%s qlwapp-%s qlwapp-%s qlwapp-%s qlwapp-%s %s", esc_attr( $button['layout'] ), esc_attr( $button['position'] ), esc_attr( $qlwapp['display']['devices'] ), esc_attr( $button['rounded'] === 'yes' ? 'rounded' : 'square' ), esc_attr( $button['animation-name'] ), join( ' ', array_map( 'sanitize_html_class', (array) wp_get_current_user()->roles ) ) ); ?>">
  <div class="qlwapp-container">
    <?php if ($button['box'] === 'yes') : ?>
      <div class="qlwapp-box">
              ...
              if (count($contacts) > 0) :
                ...
                foreach ($contacts as $id => $contact) :
                  if ($display_service->is_show_view($contact['display'])) :
                    $visibility++;
              ?>
                    <a class="qlwapp-account qlwapp-<?php echo esc_attr($contact['display']['devices']); ?>" data-action="<?php echo ($contact['chat'] && $contact['type'] != 'group' ? 'chat' : 'open'); ?>" data-visibility="<?php echo esc_attr($contact['visibility']); ?>" data-timefrom="<?php echo esc_attr($contact['timefrom']); ?>" data-timeto="<?php echo esc_attr($contact['timeto']); ?>" <?php echo sprintf('data-%s="%s"', esc_attr($contact['type']), $contact['type'] == 'phone' ? qlwapp_format_phone($contact['phone']) : esc_url($contact['group'])); ?> data-timedays="<?php echo htmlentities(json_encode($contact['timedays']), ENT_QUOTES, 'UTF-8'); ?>" data-timezone="<?php echo esc_attr(qlwapp_get_timezone_offset($contact['timezone'])); ?>" data-message="<?php echo esc_html($contact['message']); ?>" data-type="<?php echo esc_attr($contact['type']); ?>" href="javascript:void(0);" target="_blank">
        ...
        <?php if (!empty($box['footer'])) : ?>
          <div class="qlwapp-footer" data-data-visibility="<?php //echo esc_html($button['data-visibility']);                     
                                                            ?>">
    ...
    ?>
      <a class="qlwapp-toggle" data-action="<?php echo ($button['box'] === 'yes' ? 'box' : 'open'); ?>" data-visibility="<?php echo esc_attr($button['visibility']); ?>" <?php echo sprintf('data-%s="%s"', esc_attr($button['type']), $button['type'] == 'phone' ? qlwapp_format_phone($button['phone']) : esc_url($button['group'])); ?> data-type="<?php echo esc_attr($button['type']); ?>" data-timefrom="<?php echo esc_attr($button['timefrom']); ?>" data-timedays="<?php echo htmlentities(json_encode($button['timedays']), ENT_QUOTES, 'UTF-8'); ?>" data-timeto="<?php echo esc_attr($button['timeto']); ?>" data-phone="<?php echo esc_attr($button['phone']); ?>" data-timezone="<?php echo esc_attr(qlwapp_get_timezone_offset($button['timezone'])); ?>" data-message="<?php echo esc_html($button['message']); ?>" href="javascript:void(0);" target="_blank">
        <?php if ($button['icon']) : ?>
